...
import cromwell.docker.{DockerInfoRequest, DockerHashResult}
...
class EmptyDockerHashActor extends Actor {
  override def receive: Receive = {
    case request: DockerInfoRequest => sender ! DockerHashSuccessResponse(DockerHashResult("alg", "hash"), None, request)
  }
}
