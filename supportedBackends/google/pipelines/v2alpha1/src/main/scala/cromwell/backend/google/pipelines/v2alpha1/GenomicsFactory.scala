  ...
  override def build(initializer: HttpRequestInitializer): PipelinesApiRequestFactory = new PipelinesApiRequestFactory {
    ...
    override def runRequest(createPipelineParameters: CreatePipelineParameters, jobLogger: JobLogger) = {
      ...
      val adjustedBootDiskSize = {
        /*
         * At the moment, google/cloud-sdk:slim (664MB on 12/3/18) and possible stedolan/jq (182MB).
         * ~= 1 GB, + an extra 1GB to try and fix this issue https://github.com/broadinstitute/cromwell/issues/4336 which
         * according to google can be caused by a boot disk size running out of disk space and gsutil failing to write
         * authentication information to disk, hence causing failures.
         */
        ...
        // If the size of the image is obviously larger than what is in the runtime attributes, adjust it to uncompressed size
