...
import cromwell.docker.DockerInfoActor.{DockerSize, DockerHashFailureResponse, DockerHashSuccessResponse}
import cromwell.docker.{DockerClientHelper, DockerInfoRequest, DockerHashResult, DockerImageIdentifier}
...
class WorkflowDockerLookupActor private[workflow](workflowId: WorkflowId,
  ...
  when(Running) {
    ...
    case Event(request: DockerInfoRequest, data) if data.mappings.contains(request.dockerImageID) =>
      sender ! DockerHashSuccessResponse(data.mappings(request.dockerImageID).hash, None, request)
      stay()
  ...
  private def loadCacheAndHandleHashRequests(hashEntries: Map[String, DockerHashStoreEntry], data: WorkflowDockerLookupActorData): State = {
    val dockerMappingsTry = hashEntries map {
      case (dockerTag, entry) => (
        ...
        DockerHashResult.fromString(entry.dockerHash) map { hash => DockerImageMapping(hash, entry.dockerSize.map(DockerSize.apply)) }
      )
    }
    ...
    TryUtil.sequenceKeyValues(dockerMappingsTry) match {
      case Success(dockerMappings) =>
        ...
        hasMappings foreach { case (request, replyTos) =>
          ...
          replyTos foreach { _ ! DockerHashSuccessResponse(result.hash, result.dockerCompressedSize, request)}
        }
  ...
  private def persistDockerHash(response: DockerHashSuccessResponse, data: WorkflowDockerLookupActorData): Unit = {
    val dockerHashStoreEntry = DockerHashStoreEntry(workflowId.toString, response.request.dockerImageID.fullName, response.dockerHash.algorithmAndHash, response.dockerCompressedSize.map(_.compressedSize))
...
object WorkflowDockerLookupActor {
  ...
  final case class DockerImageMapping(hash: DockerHashResult, dockerCompressedSize: Option[DockerSize])

  final case class WorkflowDockerLookupActorData(hashRequests: Map[DockerInfoRequest, List[ActorRef]],
                                                 mappings: Map[DockerImageIdentifier, DockerImageMapping],
